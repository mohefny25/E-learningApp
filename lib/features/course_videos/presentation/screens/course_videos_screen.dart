...
class CourseVideosScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
  create: (context) => CourseVideosCubit(CourseVideosRepo())..getCourseVideos(courseId),
  ...
  builder: (context, state) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('course videos',style: AppTextStyles.s20w700(context)
            ),
        centerTitle: true,
        leading: InkWell(
      ...
      body: state is GetCourseVideosLoadingState?
      ...
      ListView.builder(
        ...
        itemCount: state.courseVideos.length,
          ...
          return Padding(
            padding:  EdgeInsets.symmetric(horizontal: width*.03,vertical: height*.015),
            child: Container(
                ...
                child: Row(
                  children: [
                    ClipRRect(
                      ...
                      child: Image.network(state.courseVideos[index].image??'https://img.magnific.com/premium-vector/default-image-icon-vector-missing-picture-page-website-design-mobile-app-no-photo-available_87543-11093.jpg',
                      height: height*0.08,
                        width: width*0.15,
                        fit: BoxFit.fill,
                      ),
                    ...
                    Column(
                      children: [
                        Text(state.courseVideos[index].title,style: AppTextStyles.s16w400(context).copyWith(
                          fontWeight: FontWeight.w700
                        ),),
                        Text(state.courseVideos[index].desc??"",style: AppTextStyles.s12w600(context).copyWith(
                    ...
                    InkWell(
                      onTap: (){
                        navigateTo(context, VideoPlayerScreen(courseVideoModel: state.courseVideos[index]));
                      },
                        child: CircleAvatar(
                          radius: 13,
                          backgroundColor: AppColors.primaryColor,
